...
## 11. Скриншоти роботи Postman Flow

### 1. Створення Flow з авторизацією
На рисунку нижче показано повний ланцюжок запитів у Postman Flow:  
реєстрація користувача, авторизація, збереження токена та подальші запити до захищених ендпоінтів.

![Скрін 11.1 — Схема Postman Flow для авторизації](./assets/flow-auth-diagram.jpg)  
*Скрін 11.1 — Структура Flow: реєстрація користувача, авторизація, отримання токена та виконання захищених запитів.*

### 2. Логи виконання Flow
Скріншот демонструє успішне послідовне виконання всіх кроків Flow:  
створення користувача, авторизація, отримання токена та виконання запитів до `/users`, `/categories`, `/records`.

![Скрін 11.2 — Журнал виконання Flow у Postman](./assets/flow-auth-run.jpg)  
*Скрін 11.2 — Усі кроки Flow успішно виконані: POST /auth/register → POST /auth/login → читання токена → GET /users, /categories, /records.*
